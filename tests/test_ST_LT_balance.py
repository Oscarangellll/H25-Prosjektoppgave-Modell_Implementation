...
from classes import MaintenanceCategory, VesselType, WindFarm
...
wind_farms = [
    WindFarm("Wind Farm A", n_turbines=100, location=None, distance_to_base=30, weather_data_file=None)
]
...
scenarios = [1]
failures = {
    ("Wind Farm A", "Small", 1, 1): 0,
    ("Wind Farm A", "Small", 2, 1): 1,
    ("Wind Farm A", "Small", 3, 1): 0,
    ("Wind Farm A", "Small", 4, 1): 0,
    ("Wind Farm A", "Small", 5, 1): 0,
    ("Wind Farm A", "Small", 6, 1): 1, #switch to 0 to see
...
# Expected output when failure occurs on day 6:
# Charter CTV Long Term
# Fix failure on day 2 in January
# Fix failure on day 6 in February
# Objective value: self.day_rate * days_in_period + self.mob_rate + 2*2 (operational cost)

# Expected output when failure does not occur on day 6:
